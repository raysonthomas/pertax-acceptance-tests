...
object SAGGActions {
  ...
  def logInLocalEnv(user: String, pass: String, sautr: String, nino:String)(implicit webDriver: WebDriver) = {
    ...
    if (sautr != "")
    {
    webDriver.findElement(By.xpath(".//*[@id='inputForm']/div/div[5]/div[1]/input")).sendKeys(sautr)
    }
    if (sautr != "")
    {
