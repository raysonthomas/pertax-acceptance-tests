    ...
    () =>
      provisioningCurrentDriver { implicit webDriver =>
        ...
        if (webDriver.getPageSource.contains("End contact")){
          ...
          webDriver.findElement(By.id("radio-1")).click()
          webDriver.findElement(By.id("auto_id_confirm_submit")).click()
          (new WebDriverWait(webDriver, Configuration("defaultWait").toInt).until(CustomExpectedConditions.linkVisible("Return to my Trusted Helper contacts")))
//            urlEndsWith("/help-cancelled")))
