...
object Configuration {
  ...
  lazy val environment = System.getProperty("environment", "local").toLowerCase match {
    case "local" => Map("id" -> "local", "url" -> "http://localhost:9232/personal-account",              "defaultWait" -> defaultWait)
    case "dev"   => Map("id" -> "dev",   "url" -> "https://www-dev.tax.service.gov.uk/personal-account", "defaultWait" -> defaultWait)
    case "qa"    => Map("id" -> "qa",    "url" -> "https://web-qa.tax.service.gov.uk/personal-account",  "defaultWait" -> defaultWait)
